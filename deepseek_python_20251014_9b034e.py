...
CALLBACK_DATA_FILE = "callback_data.json"
...
# --- Enhanced Authorized Users Management ---
class AuthorizedUsers:
    """Manage authorized users with JSON persistence"""
    def __init__(self):
        ...
        try:
            if os.path.exists(AUTHORIZED_USERS_FILE):
                with open(AUTHORIZED_USERS_FILE, "r") as f:
                    ...
                    self.allowed_users.add(self.admin_id)
                ...
                logger.info("✅ Created new authorized users file with admin")
    ...
    def remove_user(self, user_id):
        """Remove a user from authorized list (cannot remove admin)"""
...
# Global authorized users manager
authorized_users = AuthorizedUsers()

# --- Improved Callback Data Management ---
class CallbackData:
    """Manage callback data to avoid exceeding 64-byte limit with persistence"""
    def __init__(self):
    ...
    def load_persistent_data(self):
        """Load persistent data from file if exists"""
        try:
            if os.path.exists(CALLBACK_DATA_FILE):
                with open(CALLBACK_DATA_FILE, "r") as f:
                    data = json.load(f)
                    self.file_map = data.get("file_map", {})
                    self.next_id = data.get("next_id", 1)
                logger.info(f"✅ Loaded {len(self.file_map)} callback mappings")
            else:
                # Initialize empty file
                self.save_persistent_data()
                logger.info("✅ Created new callback data file")
        except Exception as e:
            logger.error(f"❌ Failed to load callback data: {e}")
            self.file_map = {}
            self.next_id = 1
    ...
    def save_persistent_data(self):
        ...
        try:
            ...
            with open(CALLBACK_DATA_FILE, "w") as f:
                json.dump(data, f, indent=2)
        except Exception as e:
            logger.error(f"❌ Failed to save callback data: {e}")
    ...
    def store_file(self, filename):
        ...
        # Smart cleanup: only remove oldest entries if we have too many
        if len(self.file_map) > 5000:  # Increased limit
            # Remove oldest 1000 entries
            keys_to_remove = list(self.file_map.keys())[:1000]
            for key in keys_to_remove:
                del self.file_map[key]
            logger.info(f"🧹 Cleaned up {len(keys_to_remove)} old callback mappings")
    ...
    def get_file(self, short_id):
        ...
        filename = self.file_map.get(short_id)
        if not filename:
            logger.warning(f"❌ Callback data not found for ID: {short_id}")
        return filename
    ...
    def clear_file(self, short_id):
        ...
        if short_id in self.file_map:
            ...
            self.save_persistent_data()
            logger.info(f"🧹 Cleared callback mapping for ID: {short_id}")
...
def is_authorized(func):
    ...
    @wraps(func)
    async def wrapper(client, message):
        if authorized_users.is_authorized(message.from_user.id):
            await func(client, message)
        else:
...
async def handle_callback_query(client, callback_query):
    ...
    try:
        ...
        if not filename:
            await callback_query.answer("❌ File data expired or not found", show_alert=True)
            return
        
        logger.info(f"Callback: {action} for file: {filename}")
        
        if action == "cd":  # Copy Direct
            if not authorized_users.is_authorized(user_id):
...
async def add_user_handler(client: Client, message: Message):
    try:
        ...
        authorized_users.add_user(user_id_to_add)
...
async def remove_user_handler(client: Client, message: Message):
    try:
        ...
        if authorized_users.remove_user(user_id_to_remove):
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("👥 List Users", callback_data="list_users")],
                [InlineKeyboardButton("🔙 Admin Panel", callback_data="admin_panel")]
            ])
            ...
            await message.reply_text("🤷 User not found in the authorized list or cannot remove admin.")
    except (IndexError, ValueError):
...
async def list_users_handler(client: Client, message: Message):
    users = authorized_users.get_all_users()
...
async def stats_handler(client: Client, message: Message):
    ...
    stats_text = (
        ...
        f"• Authorized users: {len(authorized_users.get_all_users())}\n"
        ...
        f"• Callback mappings: {len(callback_data.file_map)}"
    )
        ...
        try:
            ...
            if is_video_file(filename):
                ...
                if presigned_url:
                    ...
                    # Store filename for callback data
                    file_id = callback_data.store_file(filename)
...
if __name__ == "__main__":
    ...
    logger.info(f"   - Authorized users: {len(authorized_users.get_all_users())}")
    logger.info(f"   - Callback mappings loaded: {len(callback_data.file_map)}")
