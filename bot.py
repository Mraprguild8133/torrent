...
def handle_wasabi_sync(func, *args, **kwargs):
    ...
    try:
        ...
        if error_code == 'NoSuchKey':
            ...
            raise WasabiBotError("Invalid Wasabi access key")
        elif error_code == 'SignatureDoesNotMatch':
            raise WasabiBotError("Invalid Wasabi secret key")
        elif error_code == '403':
            raise WasabiBotError("Forbidden - check your credentials and bucket permissions")
        else:
            raise WasabiBotError(f"Wasabi error: {error_code} - {e.response['Error']['Message']}")
    except Exception as e:
        raise WasabiBotError(f"Wasabi connection error: {e}")
...
# Initialize Boto3 S3 Client for Wasabi with better configuration
def initialize_wasabi_client():
    try:
        ...
        print("✅ Successfully connected to Wasabi and bucket is accessible.")
        return s3_client
        ...
        error_code = e.response['Error']['Code']
        if error_code == '404':
            print(f"❌ Bucket '{config.WASABI_BUCKET}' not found. Please create it in Wasabi console.")
        elif error_code == '403':
            print("❌ Access denied. Please check your credentials and bucket permissions.")
        else:
...
s3_client = initialize_wasabi_client()
...
# Fixed Boto3 Progress Class
class Boto3Progress:
    """Callback class to display Boto3 upload/download progress."""
    def __init__(self, message, file_size, start_time, action="Processing"):
    ...
    def __call__(self, bytes_amount):
        ...
        # Use async to sync for message editing
        try:
            # We need to run this in the event loop, but we're in a sync context
            # So we'll use a thread-safe approach
            if hasattr(self._message, 'edit_text'):
                # Create a new event loop for this thread
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                loop.run_until_complete(self._message.edit_text(progress_str))
                loop.close()
        except Exception:
            # If we can't update the progress, just continue
            pass
            
        self._last_update_time = current_time

# --- Simplified Upload Function ---
async def upload_to_wasabi_simple(file_path: str, filename: str, status_msg: Message) -> str:
    """Simple upload function without complex progress tracking"""
    if not s3_client:
        raise WasabiBotError("Wasabi client is not initialized.")
    
    try:
        file_size = os.path.getsize(file_path)
        
        # Update status once at start
        await status_msg.edit_text(f"📤 Uploading to Wasabi...\n**File:** {filename}\n**Size:** {humanbytes(file_size)}")
        
        # Simple upload without progress callback
        ...
        # Generate URL
        upload_url = f"https://{config.WASABI_BUCKET}.s3.{config.WASABI_REGION}.wasabisys.com/{filename}"
        return upload_url
        
    except Exception as e:
        raise WasabiBotError(f"Upload failed: {str(e)}")

# --- Upload Function with Fixed Progress ---
async def upload_to_wasabi(file_path: str, filename: str, status_msg: Message) -> str:
    """Upload file to Wasabi with fixed progress tracking"""
    if not s3_client:
        raise WasabiBotError("Wasabi client is not initialized.")
    ...
    try:
        ...
        progress_callback = Boto3Progress(status_msg, file_size, start_time, "Uploading to Wasabi")
...
async def start_command(client, message: Message):
    status_text = "✅ Wasabi connection: Working\n" if s3_client else "❌ Wasabi connection: Failed\n"
    
    await message.reply_text(
        f"**Welcome to the Wasabi Upload Bot!**\n\n"
        f"{status_text}"
        ...
        "3. To set a **filename prefix**, use `/prefix your_prefix_`.\n"
        "4. To set a **custom thumbnail**, reply to an image with `/setthumb`.\n"
...
async def test_wasabi_command(client, message: Message):
    ...
    try:
        ...
        await message.reply_text("✅ Wasabi connection is working properly!")
    except WasabiBotError as e:
        await message.reply_text(f"❌ Wasabi connection test failed: {e}")
...
async def handle_file(client, message: Message):
    if not s3_client:
        await message.reply_text("❌ Wasabi client is not initialized. Please check your credentials.")
        return
    ...
    final_filename = sanitize_filename(f"{prefix}{filename}")

    os.makedirs(config.DOWNLOAD_PATH, exist_ok=True)

    # Download from Telegram
    status_msg = await message.reply_text(f"📥 Downloading `{final_filename}`...", quote=True)
    start_time = time.time()
    ...
    try:
        ...
        if not download_path or not os.path.exists(download_path):
            raise WasabiBotError("Downloaded file not found")
            
    except WasabiBotError as e:
        await status_msg.edit_text(f"❌ Error downloading: {e}")
        return

    # Upload to Wasabi - Use simple version first for testing
    await status_msg.edit_text("☁️ Preparing to upload to Wasabi...")
    
    try:
        # Try simple upload first (without progress)
        upload_url = await upload_to_wasabi_simple(download_path, final_filename, status_msg)
...
async def main():
    ...
    if s3_client:
        try:
            ...
            print(f"⚠️  Wasabi bucket access issue: {e}")
