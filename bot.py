...
def generate_streaming_urls(file_name, presigned_url):
    ...
    # Clean and validate the presigned URL
    presigned_url = validate_url(presigned_url)
    
    # Online HTML player URL - keep it short
    html_player_url = f"{BASE_URL}/player.html"
    
    # For MX Player and VLC, we'll use callback to avoid long URLs
    return {
        'online': html_player_url,
        'direct': presigned_url,
        'file_name': file_name,
        'encoded_url': urllib.parse.quote(presigned_url) if presigned_url else ''
    }
...
async def file_handler(client, message: Message):
    ...
    try:
        ...
        # Use callback data to avoid long URLs in buttons
        if is_video_streamable:
...
async def online_player_callback(client, callback_query):
    ...
    # Send the online player link
    await callback_query.message.reply_text(
        f"**🎬 Online Player for `{file_name}`**\n\n"
        f"Click the link below to open the video in your browser:\n\n"
        f"[Open Online Player]({online_url})",
...
async def mx_player_callback(client, callback_query):
    ...
    await callback_query.message.reply_text(
        ...
        f"Use the button below to open in MX Player, or use this URL:\n\n"
        f"`{mx_url}`",
        reply_markup=InlineKeyboardMarkup([
...
async def vlc_player_callback(client, callback_query):
    ...
    await callback_query.message.reply_text(
        ...
        f"Use the button below to open in VLC Player, or use this URL:\n\n"
        f"`{vlc_url}`",
        reply_markup=InlineKeyboardMarkup([
...
async def streaming_links_callback(client, callback_query):
    ...
    stream_message = (
        ...
        f"• 🌐 [Online HTML5 Player]({online_url}) - Works in any browser\n"
        f"• 📱 [MX Player]({mx_url}) - Best for Android\n"
        f"• ▶️ [VLC Player]({vlc_url}) - Works on all platforms\n"
        f"• 🔗 [Direct URL]({presigned_url}) - Use with any player\n\n"
        f"**Instructions:**\n"
        f"• Click the links above or use the copy buttons below"
    )
    
    markup = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("🌐 Online", callback_data=f"online_{file_id}"),
            InlineKeyboardButton("📱 MX", callback_data=f"mx_{file_id}"),
            InlineKeyboardButton("▶️ VLC", callback_data=f"vlc_{file_id}")
        ],
        [InlineKeyboardButton("🔗 Direct URL", url=presigned_url)]
    ])
    
    await callback_query.message.reply_text(stream_message, reply_markup=markup)
    await callback_query.answer("Streaming options sent!")
