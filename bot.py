...
def generate_enhanced_player_url(filename: str, presigned_url: str, file_type: str = None) -> Optional[str]:
    """Generate enhanced player URL with configuration options."""
    if not RENDER_URL:
        return None
    ...
    encoded_params = base64.urlsafe_b64encode(
        json.dumps(player_params).encode()
    ).decode().rstrip('=')
    
    return f"{RENDER_URL}/player/{file_type}/{encoded_params}"

def generate_direct_player_url(filename: str, presigned_url: str) -> Optional[str]:
    """Generate direct player URL (backward compatible)."""
    if not RENDER_URL:
        return None
    ...
    if is_video_file(filename):
        ...
        encoded_url = base64.urlsafe_b64encode(presigned_url.encode()).decode().rstrip('=')
        return f"{RENDER_URL}/player/audio/{encoded_url}"
    elif is_image_file(filename):
        encoded_url = base64.urlsafe_b64encode(presigned_url.encode()).decode().rstrip('=')
        return f"{RENDER_URL}/player/image/{encoded_url}"
    return None
...
async def generate_player_links(filename: str, presigned_url: str) -> Dict[str, str]:
    ...
    links = {
        ...
        'player_legacy': generate_direct_player_url(filename, presigned_url),
        'player_enhanced': generate_enhanced_player_url(filename, presigned_url)
    }
    
    # Determine the best player URL to use
    if links['player_enhanced']:
        links['player_primary'] = links['player_enhanced']
    else:
        links['player_primary'] = links['player_legacy']
...
def register_handlers(client):
    ...
    async def start_handler(_, message: Message):
        welcome_text = (
            ...
            f"• Enhanced web player available at: {RENDER_URL}"
        )
        ...
        keyboard = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("🎮 Enhanced Player", url=RENDER_URL)],
...
• Video/Audio/Image files get enhanced player URLs
• All links are valid for 7 days
• Enhanced web player: {RENDER_URL}
...
**Enhanced Player URLs:**
All media files get special player URLs with enhanced features.
"""
    ...
    async def stats_handler(_, message: Message):
        stats_text = (
            ...
            f"**Player Features:**\n"
            f"• Enhanced player: {'✅' if RENDER_URL else '❌'}\n"
            f"• Video formats: {len(SUPPORTED_VIDEO_FORMATS)}\n"
            f"• Audio formats: {len(SUPPORTED_AUDIO_FORMATS)}\n"
            f"• Image formats: {len(SUPPORTED_IMAGE_FORMATS)}\n\n"
            f"**Configuration:**\n"
    ...
    async def player_url_handler(_, message: Message):
        """Generate enhanced player URL for existing files in Wasabi"""
        try:
            ...
            try:
                ...
                response_text = (
                    ...
                    f"**Type:** {file_type}\n"
                    f"**Player:** {links['player_primary']}\n"
                    f"**Direct:** `{links['direct'][:100]}...`\n\n"
    ...
    async def status_callback(_, query):
        status_text = (
            ...
            f"**Enhanced Player:** {RENDER_URL}\n"
            f"**Supported Formats:**\n"
            f"• Videos: {len(SUPPORTED_VIDEO_FORMATS)}\n"
            f"• Audio: {len(SUPPORTED_AUDIO_FORMATS)}\n"
            f"• Images: {len(SUPPORTED_IMAGE_FORMATS)}\n\n"
            f"*Send a file to test enhanced player*"
        )
...
async def start_web_server():
    ...
    try:
        ...
        web_thread.start()
        logger.info(f"🌐 Enhanced web server started on port {WEB_PORT}")
        return True
    except Exception as e:
...
async def main():
    ...
    try:
        ...
        logger.info(f"🌐 Enhanced web server: {'Running on port ' + str(WEB_PORT) if web_server_ready else 'Not available'}")
        logger.info(f"🎮 Enhanced player available at: {RENDER_URL}")
