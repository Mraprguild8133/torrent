...
def create_link_buttons(direct_url, player_url, filename):
    """Create beautiful inline buttons for links"""
    buttons = []
    ...
    if direct_url:
        buttons.append([
            InlineKeyboardButton("📋 Copy Direct Link", callback_data=f"copy_direct_{filename}"),
            InlineKeyboardButton("📋 Copy Player Link", callback_data=f"copy_player_{filename}")
        ])
    
    # Add admin buttons for admin users
    buttons.append([
        InlineKeyboardButton("🗑 Delete File", callback_data=f"delete_{filename}"),
        InlineKeyboardButton("🔄 New Links", callback_data=f"refresh_{filename}")
    ])
    
    return InlineKeyboardMarkup(buttons)

def create_simple_buttons(direct_url, player_url):
    ...
    if direct_url:
        buttons.append([
            InlineKeyboardButton("📋 Copy Direct", callback_data="copy_direct_simple"),
            InlineKeyboardButton("📋 Copy Player", callback_data="copy_player_simple")
        ])
...
async def handle_callback_query(client, callback_query):
    ...
    try:
        if data.startswith('copy_direct_'):
            ...
            filename = data.replace('copy_direct_', '')
            presigned_url = await generate_presigned_url(filename)
            
            if presigned_url:
                # Copy to clipboard (simulated)
                await callback_query.answer("📋 Direct link copied to clipboard!", show_alert=True)
                # Also send the link as a message for easy access
            ...
            filename = data.replace('copy_player_', '')
            presigned_url = await generate_presigned_url(filename)
            player_url = generate_player_url(filename, presigned_url) if presigned_url else None
            
            if player_url:
                await callback_query.answer("📋 Player link copied to clipboard!", show_alert=True)
                await message.reply_text(
                    f"**Player URL:**\n{player_url}",
                    reply_to_message_id=message.id
                )
            else:
                await callback_query.answer("❌ Failed to generate player link", show_alert=True)
            ...
            filename = data.replace('refresh_', '')
            ...
            if presigned_url:
                ...
                await callback_query.answer("✅ Links refreshed!", show_alert=True)
            else:
                await callback_query.answer("❌ Failed to refresh links", show_alert=True)
                
        elif data == "copy_direct_simple":
            # For simple users, just show the link
            await callback_query.answer("📋 Check your messages for the direct link!", show_alert=True)
            
        elif data == "copy_player_simple":
            await callback_query.answer("📋 Check your messages for the player link!", show_alert=True)
...
# --- Ultra-Fast File Handling with Buttons ---
...
async def file_handler(client: Client, message: Message):
    ...
    try:
        ...
        if message.from_user.id == ADMIN_ID:
            ...
            keyboard = create_simple_buttons(presigned_url, player_url)
