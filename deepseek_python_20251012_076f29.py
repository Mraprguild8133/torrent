...
def generate_player_url(filename: str, presigned_url: str) -> Optional[str]:
    ...
    try:
        # Double encode the filename for safety in URL
        safe_filename = quote(filename, safe='')
        
        if is_video_file(filename):
            return f"{RENDER_URL.rstrip('/')}/player/video?url={quote(presigned_url)}&filename={safe_filename}"
        elif is_audio_file(filename):
            return f"{RENDER_URL.rstrip('/')}/player/audio?url={quote(presigned_url)}&filename={safe_filename}"
        return None
...
def generate_direct_link(file_name: str) -> str:
    ...
    try:
        ...
        # Use the correct Wasabi public URL format
...
def test_url_access(url: str, filename: str) -> bool:
    """Test if a URL is accessible."""
    try:
        import requests
        response = requests.head(url, timeout=10)
        if response.status_code == 200:
            logger.info(f"✅ URL test passed for {filename}")
            return True
        else:
            logger.warning(f"⚠️ URL test failed for {filename}: Status {response.status_code}")
            return False
    except Exception as e:
        logger.warning(f"⚠️ URL test error for {filename}: {e}")
        return False
...
def register_handlers(client):
    ...
    async def start_handler(_, message: Message):
        welcome_text = (
            ...
            "• Video/Audio player URLs for streaming\n"
...
• Video/Audio files get player URLs for streaming
...
"""
    ...
    async def player_url_handler(_, message: Message):
        ...
        try:
            ...
            try:
                ...
                direct_link = generate_direct_link(filename)
                ...
                # Prepare response
                file_type = "Video" if is_video_file(filename) else "Audio" if is_audio_file(filename) else "File"
                ...
                keyboard_buttons.append([InlineKeyboardButton("🌐 Direct Link", url=direct_link)])
                
                # Add presigned URL button
                keyboard_buttons.append([InlineKeyboardButton("🔗 Download Link", url=presigned_url)])
                
                # Add player URL if available
                if player_url:
                    response_text += f"**Player URL:** `{player_url}`\n\n"
                    player_icon = "🎥" if is_video_file(filename) else "🎵"
                    keyboard_buttons.append([InlineKeyboardButton(f"{player_icon} Web Player", url=player_url)])
                
                response_text += "*Links valid for 7 days*"
    ...
    async def file_handler(client, message: Message):
        ...
        try:
            ...
            player_url = generate_player_url(file_name, presigned_url) if presigned_url else None
            ...
            # Show download/presigned URL button
            keyboard_buttons.append([InlineKeyboardButton("🔗 Download Link", url=presigned_url)])
            
            # Show player URL for videos and audio
            if player_url:
                player_icon = "🎥" if is_video_file(file_name) else "🎵"
                keyboard_buttons.append([InlineKeyboardButton(f"{player_icon} Web Player", url=player_url)])
            
            # Store URLs for callback buttons
            if presigned_url:
            ...
            # Add web player link
            if RENDER_URL:
                keyboard_buttons.append([InlineKeyboardButton("🌐 Open Web Player", url=RENDER_URL)])
