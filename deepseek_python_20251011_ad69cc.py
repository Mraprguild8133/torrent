...
class CallbackDataManager:
    """Manage callback data to avoid exceeding Telegram limits"""
    
    def __init__(self):
        self.callback_store = {}
        self.callback_counter = 0
    
    def generate_callback_id(self, action: str, data: str) -> str:
        """Generate short callback ID"""
        callback_id = f"cb{self.callback_counter}"
        self.callback_store[callback_id] = {'action': action, 'data': data}
        self.callback_counter += 1
        
        # Clean old callbacks to prevent memory leaks
        if len(self.callback_store) > 1000:
            self._clean_old_callbacks()
        
        return callback_id
    
    def get_callback_data(self, callback_id: str) -> Optional[Dict]:
        """Get original callback data"""
        return self.callback_store.get(callback_id)
    
    def _clean_old_callbacks(self):
        """Clean old callbacks to prevent memory leaks"""
        # Keep only the last 500 callbacks
        if len(self.callback_store) > 500:
            keys_to_remove = list(self.callback_store.keys())[:500]
            for key in keys_to_remove:
                del self.callback_store[key]

class AsyncWasabiClient:
    """Async Wasabi S3 client"""
    
    def __init__(self):
        self.session = aioboto3.Session()
        self.config = config
        
    async def upload_file(self, file_path: str, filename: str, progress_callback: Callable = None):
        """Upload file to Wasabi with async support"""
        async with self.session.client(
            's3',
            endpoint_url=self.config.wasabi_endpoint,
            aws_access_key_id=self.config.WASABI_ACCESS_KEY,
            aws_secret_access_key=self.config.WASABI_SECRET_KEY,
            region_name=self.config.WASABI_REGION
        ) as s3:
            file_size = os.path.getsize(file_path)
            if file_size < self.config.CHUNK_SIZE:
                # Simple upload for small files
                await s3.upload_file(file_path, self.config.WASABI_BUCKET, filename)
            else:
                # Multipart upload for large files
                await self._multipart_upload(s3, file_path, filename, file_size, progress_callback)
    
    async def _multipart_upload(self, s3, file_path: str, filename: str, file_size: int, progress_callback: Callable):
        """Multipart upload for large files"""
        # Create multipart upload
        mpu = await s3.create_multipart_upload(
            Bucket=self.config.WASABI_BUCKET,
            Key=filename
        )
    ...
    async def download_file(self, filename: str, local_path: str, progress_callback: Callable = None):
        """Download file from Wasabi with async support"""
        async with self.session.client(
            's3',
            endpoint_url=self.config.wasabi_endpoint,
            aws_access_key_id=self.config.WASABI_ACCESS_KEY,
            ...
            # Get file size first
            head_response = await s3.head_object(Bucket=self.config.WASABI_BUCKET, Key=filename)
            file_size = head_response['ContentLength']
            
            # Download file
            response = await s3.get_object(Bucket=self.config.WASABI_BUCKET, Key=filename)
            async with response['Body'] as stream:
                async with aiofiles.open(local_path, 'wb') as file:
                    ...
                    async for chunk in stream.iter_chunks():
                        ...
                        if progress_callback:
                            await progress_callback(bytes_downloaded, file_size)
    ...
    async def list_files(self):
        ...
        async with self.session.client(
            ...
            response = await s3.list_objects_v2(Bucket=self.config.WASABI_BUCKET)
    ...
    async def file_exists(self, filename: str) -> bool:
        ...
        async with self.session.client(
            ...
            try:
                await s3.head_object(Bucket=self.config.WASABI_BUCKET, Key=filename)
                return True
            except ClientError:
                return False
    ...
    async def delete_file(self, filename: str):
        ...
        async with self.session.client(
            ...
            await s3.delete_object(Bucket=self.config.WASABI_BUCKET, Key=filename)
    ...
    async def get_file_size(self, filename: str) -> Optional[int]:
        ...
        async with self.session.client(
            ...
            try:
                response = await s3.head_object(Bucket=self.config.WASABI_BUCKET, Key=filename)
...
class WasabiTelegramBot:
    def __init__(self):
        ...
        # Initialize Async Wasabi client
        self.wasabi = AsyncWasabiClient()
        
        # Initialize Callback Data Manager
        ...
        self.progress_locks: Dict[int, Lock] = {}
        ...
        async def start_command(client, message: Message):
            await message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("📤 Upload Help", callback_data="upload_help")],
                    [InlineKeyboardButton("📥 Download Help", callback_data="download_help")]
                ])
            )
    ...
    async def upload_file_to_wasabi(self, message: Message):
        ...
        try:
            ...
            # Generate short callback IDs for buttons
            delete_callback = self.callback_manager.generate_callback_id("delete", final_filename)
            list_callback = self.callback_manager.generate_callback_id("list", "")
            
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("🌐 Download URL", url=download_url)],
                [InlineKeyboardButton("🗑 Delete File", callback_data=delete_callback)],
                [InlineKeyboardButton("📁 List Files", callback_data=list_callback)]
            ])
    ...
    async def download_telegram_file(self, message: Message, file_obj, status_msg) -> Optional[str]:
        ...
        try:
            ...
            temp_path = download_dir / f"temp_{message.id}"
            ...
            async def progress(current, total):
                nonlocal downloaded
                downloaded = current
                await self.update_progress(status_msg, current, total, "Downloading from Telegram")
    ...
    async def download_file_from_wasabi(self, message: Message, filename: str):
        ...
        try:
            ...
            if not await self.wasabi.file_exists(filename):
                await status_msg.edit_text(f"❌ File `{filename}` not found in Wasabi storage")
                return
            
            # Download from Wasabi with progress
            await self.wasabi.download_file(
                filename,
                str(local_path),
                progress_callback=lambda current, total: self.update_progress(
    ...
    async def list_files(self, message: Message):
        ...
        try:
            ...
            files = await self.wasabi.list_files()
            ...
            for file in files[:30]:  # Limit to 30 files to avoid message too long
                filename = file['Key']
    ...
    async def update_progress(self, message: Message, current: int, total: int, operation: str):
        ...
        try:
            ...
            if message_id not in self.progress_locks:
                self.progress_locks[message_id] = Lock()
            
            async with self.progress_locks[message_id]:
...
async def handle_callback_query(client, callback_query):
    ...
    # Handle short callback IDs
    if data.startswith("cb"):
        callback_data = bot.callback_manager.get_callback_data(data)
        ...
        if action == "delete":
            try:
                await bot.wasabi.delete_file(file_data)
                await callback_query.answer("✅ File deleted from Wasabi")
                await callback_query.message.edit_text(
                    f"🗑 `{file_data}` has been deleted from Wasabi storage.", 
                    parse_mode=ParseMode.MARKDOWN
                )
            except Exception as e:
            ...
            await callback_query.answer("📁 Listing files...")
