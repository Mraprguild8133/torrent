...
def generate_player_url(filename: str, presigned_url: str) -> Optional[str]:
    ...
    try:
        # Properly encode the presigned URL for the web player
        ...
        if is_video_file(filename):
            return f"{RENDER_URL.rstrip('/')}/player/video/{encoded_url}"
        elif is_audio_file(filename):
            return f"{RENDER_URL.rstrip('/')}/player/audio/{encoded_url}"
        return None
...
def generate_streaming_link(file_name: str) -> str:
    ...
    try:
        ...
        # Use the correct Wasabi endpoint format
        return f"https://{WASABI_BUCKET}.s3.{WASABI_REGION}.wasabisys.com/{encoded_file_name}"
    except Exception as e:
        logger.error(f"Error generating streaming link: {e}")
        return f"https://s3.{WASABI_REGION}.wasabisys.com/{WASABI_BUCKET}/{quote(file_name)}"

def generate_presigned_url_fixed(file_name: str, expires_in: int = 604800) -> Optional[str]:
...
"""
    ...
    async def player_url_handler(_, message: Message):
        ...
        try:
            ...
            try:
                ...
                streaming_link = generate_streaming_link(filename)
                ...
                if presigned_url:
                    ...
                    if player_url:
                        keyboard = InlineKeyboardMarkup([
                            ...
                            [InlineKeyboardButton("🔗 Direct Link", url=streaming_link)],
                            [InlineKeyboardButton("📋 Presigned URL", url=presigned_url)]
                        ])
                        ...
                        await message.reply_text(
                            ...
                            f"**Type:** {file_type}\n"
                            f"**Player:** {player_url}\n"
                            f"**Direct:** `{streaming_link}`\n"
                            f"**Presigned:** `{presigned_url[:100]}...`\n\n"
                        ...
                        await message.reply_text(
                            f"🔗 **Direct URL for `{filename}`**\n\n"
                            f"**Streaming Link:** `{streaming_link}`\n"
                            f"**Presigned URL:** `{presigned_url[:100]}...`\n\n"
                            f"*This file type doesn't support web player. Use the direct links above.*"
                        )
                else:
    ...
    async def file_handler(client, message: Message):
        ...
        try:
            ...
            # Show presigned URL button
            if presigned_url:
                keyboard_buttons.append([InlineKeyboardButton("🔗 Presigned URL", url=presigned_url)])
            
            # Show player URL for videos and audio
            if player_url:
                player_icon = "🎥" if is_video_file(file_name) else "🎵"
                keyboard_buttons.append([InlineKeyboardButton(f"{player_icon} Player URL", url=player_url)])
